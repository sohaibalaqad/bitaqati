...
.add-row-btn:hover { border-color:var(--accent); color:var(--accent); background:rgba(124,58,237,.04); }
.grid-2 { display:grid; grid-template-columns:1fr 1fr; gap:14px; }
.section-type-badge {
    display:inline-flex; align-items:center; gap:6px; padding:4px 12px;
    background:rgba(124,58,237,.08); color:var(--accent); border-radius:20px;
...
<div class="edit-card">
  ...
  <form action="{{ route('superadmin.homepage.update', $section) }}" method="POST">
    @csrf @method('PUT')
    ...
    @if($section->type === 'hero')
      ...
      <div class="field-group">
        ...
        <textarea name="title" rows="3">{{ old('title', $c['title'] ?? '') }}</textarea>
    ...
    @elseif($section->type === 'pricing')
      ...
      <div style="padding:14px;background:rgba(124,58,237,.06);border-radius:8px;border:1px solid rgba(124,58,237,.15);font-size:13px;color:var(--text-muted)">
        ...
        تُدار الخطط الفعلية (الأسعار والمميزات) من صفحة <a href="{{ route('superadmin.homepage.plans') }}" style="color:var(--accent);font-weight:600">إدارة الخطط</a>.
      </div>
    ...
    <div style="display:flex;gap:10px;margin-top:28px;padding-top:20px;border-top:1px solid var(--border)">
      ...
      <a href="{{ route('superadmin.homepage.index') }}" class="btn btn-outline" style="padding:10px 24px;font-size:14px">إلغاء</a>
    </div>
  </form>
</div>
@endsection
